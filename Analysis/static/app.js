...
function initializeEventListeners() {
    ...
    // Export buttons (Universe Screener)
    ...
    document.getElementById("copyTickersButton")?.addEventListener("click", copyFilteredTickersToClipboard);

    // Matrix search inputs
    document.getElementById("matrixSearchInput")?.addEventListener("input", filterMatrixTable);
    document.getElementById("betaMatrixSearchInput")?.addEventListener("input", filterBetaMatrixTable);
...
function initializeCustomTracker() {
    ...
    // Initial Seed: Add example showcase tickers matching user's screenshot
    seedInitialTracker();
}

function seedInitialTracker() {
    customTrackerItems.set("COHR", { ticker: "COHR", actual_move: 0.04 });
    customTrackerItems.set("CRWV", { ticker: "CRWV", actual_move: 0.01 });
    customTrackerItems.set("LITE", { ticker: "LITE", actual_move: 0.06 });
...
function removeTrackerItem(ticker) {
    ...
    trackerCalculatedRows = trackerCalculatedRows.filter(r => r.ticker !== ticker);
    applyTrackerFilters();
}
...
async function calculateCustomTracker() {
    ...
    try {
        ...
        if (Array.isArray(data.ignored) && data.ignored.length > 0) {
            const ignoredMsg = `Пропущено ${data.ignored.length} тикеров (история меньше периода ${period}): ${data.ignored.join(", ")}`;
            showTrackerWarning(ignoredMsg);
        } else if (Array.isArray(data.missing) && data.missing.length > 0) {
            showTrackerWarning(`Тикеры не найдены в БД: ${data.missing.join(", ")}`);
        }
...
function renderTrackerTablePage() {
    ...
    pageRows.forEach(item => {
        ...
        tr.innerHTML = `
            <td class="asset-name-cell"><strong>${item.ticker}</strong></td>
            <td class="corr-cell">
...
function renderScreenerTablePage() {
    ...
    const colSpan = (activeBenchmarkView === "compare" && configuredBenchmarks.length > 1) ? (4 + configuredBenchmarks.length * 3) : 10;
    ...
    if (activeBenchmarkView === "compare" && configuredBenchmarks.length > 1) {
        pageRows.forEach(item => {
            const tr = document.createElement("tr");
            let tdHtml = `<td class="asset-name-cell"><strong>${item.ticker}</strong></td>`;
...
function renderCorrelationMatrix(data) {
    ...
    const symbols = data.all_symbols || Object.keys(data.correlation || {});
    const renderSymbols = symbols.slice(0, 100);
    ...
    renderSymbols.forEach(rowSym => {
        ...
        renderSymbols.forEach(colSym => {
            ...
            html += `<td style="background: ${color}">${formatNumber(val, 2)}</td>`;
        });
        html += "</tr>";
    });
...
function filterMatrixTable(e) {
    const q = normalizeTicker(e.target.value);
    const rows = document.querySelectorAll("#correlationMatrix tbody tr");
    rows.forEach(tr => {
        const header = tr.querySelector("th")?.textContent || "";
        tr.style.display = !q || header.includes(q) ? "" : "none";
    });
}
...
function renderBetaMatrix(data) {
    ...
    const symbols = data.all_symbols || Object.keys(data.beta || {});
    const renderSymbols = symbols.slice(0, 100);
    ...
    renderSymbols.forEach(rowSym => {
        ...
        renderSymbols.forEach(colSym => {
            ...
            const color = getBetaColor(val);
            html += `<td style="background: ${color}" class="${isDiag ? 'diag-cell' : ''}">${formatNumber(val, 3)}</td>`;
        });
        html += "</tr>";
    });
...
function filterBetaMatrixTable(e) {
    const q = normalizeTicker(e.target.value);
    const rows = document.querySelectorAll("#betaMatrix tbody tr");
    rows.forEach(tr => {
        const header = tr.querySelector("th")?.textContent || "";
        tr.style.display = !q || header.includes(q) ? "" : "none";
    });
